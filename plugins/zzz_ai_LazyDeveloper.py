    ...
    if AI == True: 
        ...
        if user_id:
            try:
                ...
                footer_credit = "🦋<a href='https://telegram.me/helpsarthak_bot'>• ʀᴇᴘᴏʀᴛ ɪꜱꜱᴜᴇ •</a>══<a href='https://telegram.me/sarthakkale16'>• ᴄᴏɴᴛᴀᴄᴛ ᴍᴀꜱᴛᴇʀ •</a>🦋"
                lazy_response = response.choices[0].text 
                await client.send_message(LAZY_AI_LOGS, text=f"⚡️⚡️#Lazy_AI_Query \n\n• A user named **{message.from_user.mention}** 𝚆𝚒𝚝𝚑 𝚄𝚜𝚎𝚛 𝙸𝙳 - `{user_id}`. 𝙰𝚂𝙺𝙴𝙳 𝚃𝙷𝙸𝚂..\n\n══❚█══Q   U   E   R   Y══█❚══\n\n\n[Q྿.]**{lazy_users_message}**\n\n👇Here is what i responded:\n:-`{lazy_response}`\n\n\n❚═USER ID═❚═• `{user_id}` \n❚═USER Name═❚═• `{message.from_user.mention}` \n\n🗃️" , reply_markup = reply_markup )
                await message.reply(f"{lazy_response}\n\n\n{footer_credit}")
            except Exception as error:
                print(error)
                await message.reply_text(f'𝚆𝚑𝚢 𝚃𝙵 𝙴𝚁𝚁𝙾𝚁 . 𝚁𝙴𝙿𝙾𝚁𝚃 𝙽𝙾𝚆\n\n{error}')
    else:
        return
